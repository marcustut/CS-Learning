...
function minimizeDatepicker()
{
	$('.datepicker_oneMonth table').find('thead').hide();
	$('.datepicker_oneMonth table').find('tbody').addClass('clearfix');		
	$('table.datepicker').find('tr').each(function(index){
		$(this).replaceWith($(this).html());
	});	
	$('table.datepicker').find('td').each(function(index){
		if ($(this).hasClass('datepicker_weekEndCell')){
			if((parseInt($(this).find('a').html())%2)>0)
					$(this).find('a').attr('title', 'Saturday');
			else
					$(this).find('a').attr('title', 'Sunday');
		}else{
			//do nothing
		}
	});	
